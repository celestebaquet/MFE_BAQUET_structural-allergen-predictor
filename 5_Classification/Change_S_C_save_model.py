...
def evaluate_model(X_training, y_training, X_test, y_test, k_values, selector, classifier, concat):
    ...
    for k in k_values:
        ...
        clf_k = make_pipeline(SelectKBest(selector, k=k), MinMaxScaler(), classifier)
        clf_k.fit(X_training, y_training)

        # y_pred = clf_k.predict(X_test)
        
        # Predict with threshold
        y_proba = clf_k.predict_proba(X_test)[:, 1]
        ...
        if composite_score < best_score:
            ...
            best_model = clf_k  # Enregistrer le modèle correspondant à ce k
...
if __name__ == '__main__':
    ...
    selector = f_classif
    str_selector = 'f_classif'
    # Classificator : LinearSVC(max_iter=5000), GradientBoostingClassifier(n_estimators=200, learning_rate=0.1, max_depth=5, random_state=42), xgb.XGBClassifier(n_estimators=200, learning_rate=0.1, max_depth=5, random_state=42), 
    classifier = GradientBoostingClassifier(n_estimators=200, learning_rate=0.1, max_depth=5, random_state=42)
    str_classifier = 'GB_t_3'
    step = 15
    k_start = 1
